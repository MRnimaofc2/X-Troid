...
if (config.WORKTYPE == 'private') {
   ...
   XTroid.addCMD({pattern: 'img ?(.*)', fromMe: true, desc: Lang.IMG_DESC}, (async (message, match) => { 
    ...
    XTroid.addCMD({pattern: 'songxyz ?(.*)', fromMe: false, desc: Lang.SONG_DESC}, (async (message, match) => { 
    ...
    XTroid.addCMD({pattern: 'ytmp4 ?(.*)', fromMe: false, desc: Lang.VIDEO_DESC}, (async (message, match) => { 
        ...
        try {
            if (match[1].includes('watch','shorts/')) {
                var tsts = match[1].replace('watch?v=','?feature=share', '')
                var alal = tsts.split('/')[3]
                VID = alal
            } else {     
                VID = match[1].split('/')[3]
    ...
    XTroid.addCMD({pattern: 'yts ?(.*)', fromMe: false, desc: Lang.YT_DESC}, (async (message, match) => { 
